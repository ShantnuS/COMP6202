...
def get_genotypes():
    genotypes = []
    individual = ("1"*(allele_size)).zfill(allele_size)
...
def initialise_pool():
    ...
    while len(pool)<N:
        individual = ("1"*(allele_size)).zfill(allele_size)
        individual2 = ("1"*(0)).zfill(allele_size)
        pool.append(individual2+"L")
        pool.append(individual2+"S")
    ...
    for _ in range(t):
        ...
        for individual in group:
            ...
            if individual not in reproduced_genotype:
                ...
                ni = group.count(individual)
                Gi = calculate_G(individual)
                Ci = calculate_C(individual)
                top = ni*Ci*Gi

                ri = resource_received(top, bottom, R)
                new_num = replicate(ni, ri, Ci, K)
